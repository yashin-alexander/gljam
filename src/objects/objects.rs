...
pub fn main_objects() {
    ...
    let (mut window, events) = glfw.create_window(SCR_WIDTH, SCR_HEIGHT, "objects", glfw::WindowMode::Windowed)
    ...
    let (lightingShader, lampShader, VBO, cubeVAO, lightVAO, diffuseMap, specularMap, cubePositions, pointLightPositions) = unsafe {
        ...
        let lightingShader = Shader::new(
            "src/objects/shaders/lights.vs",
            "src/objects/shaders/lights.fs");
        let lampShader = Shader::new(
            "src/objects/shaders/lamp.vs",
            "src/objects/shaders/lamp.fs");
